...
export async function handleFinancialOverview(
  ...
  return await withToolErrorHandling(async () => {
    ...
    const sortedMonths = validMonths.sort((a, b) => new Date(b.data.month.month).getTime() - new Date(a.data.month.month).getTime());
    let analysisDateRange = `${params.months} months`; // fallback
    if (sortedMonths.length > 0 && sortedMonths[0]?.data?.month?.month && sortedMonths[sortedMonths.length - 1]?.data?.month?.month) {
    ...
    const overview = {
      ...
      spending_trends: {
        ...
        explanation: "Trends are calculated using Z-score anomaly detection and percentile analysis to identify genuinely unusual spending patterns. Categories need at least 3 months of data. Higher variability categories require stronger deviations to trigger alerts, reducing false alarms.",
        confidence_levels: {
...
export async function handleBudgetHealthCheck(
  ...
  return await withToolErrorHandling(async () => {
    ...
    const analysisDateRange = analysisDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
...
function analyzeSpendingTrends(months: MonthData[], categories: ynab.Category[]): SpendingTrend[] {
  ...
  spendingCategories.forEach(category => {
    ...
    if (monthlySpending.length >= 3) {
      ...
      const p90 = quantile(sortedSpending, 0.90) || 0;
      ...
      const simplePercentChange = previousPeriod > 0 ? ((currentPeriod - previousPeriod) / previousPeriod) * 100 : 0;
      ...
      if (coefficientOfVariation < 20) { // Low variability categories
      ...
      } else if (coefficientOfVariation < 50) { // Medium variability categories
      ...
      } else { // High variability categories - very conservative
...
function generateFinancialInsights(
  ...
  if (overspentCategories.length > 0) {
    insights.push({
      ...
      impact: 'high',
      actionable: true,
      suggestions: ['Move money from other categories immediately', 'Reduce spending in overspent categories', 'Adjust budget allocations for next month'],
    });
  }
  ...
  const exceededMonthlyBudget = budget.categories?.filter(category => {
    if (category.name.toLowerCase().includes('inflow:') || 
        category.name.toLowerCase().includes('ready to assign')) {
      return false;
    }
    ...
    const activity = Math.abs(ynab.utils.convertMilliUnitsToCurrencyAmount(monthCategory.activity));
  ...
  if (exceededMonthlyBudget.length > 0) {
    ...
    insights.push({
      ...
      suggestions: [
        ...
        `Categories: ${exceededMonthlyBudget.slice(0, 3).map(c => c.name).join(', ')}${exceededMonthlyBudget.length > 3 ? '...' : ''}`
      ],
    });
  }
...
function generateBudgetOptimizationInsights(
  ...
  const largeUnusedBalances = currentMonth.categories
    ?.filter(cat => {
      ...
      if (!category || category.name.toLowerCase().includes('inflow:') || 
          category.name.toLowerCase().includes('ready to assign')) {
        return false;
      }
  ...
  if (largeUnusedBalances.length > 0) {
    ...
    insights.push({
      ...
      suggestions: [
        ...
        `Largest balances: ${largeUnusedBalances.slice(0, 3).map(c => `${c.name} ($${formatCurrency(c.balance)})`).join(', ')}`
      ],
    });
  }
...
function performDetailedSpendingAnalysis(months: MonthData[], categories: ynab.Category[], categoryId?: string) {
  const targetCategories = categoryId 
    ? categories.filter(c => c.id === categoryId)
    : categories;

  // Generate clear date range
  const sortedMonths = months.sort((a, b) => new Date(b.data.month.month || '').getTime() - new Date(a.data.month.month || '').getTime());
  const startDate = new Date(sortedMonths[sortedMonths.length - 1]?.data.month.month || '');
  const endDate = new Date(sortedMonths[0]?.data.month.month || '');
  ...
  const categoryAnalysis = targetCategories.map(category => {
...
function performBudgetHealthCheck(
  ...
  return {
    analysis_period: analysisDateRange || new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' }),
    health_score: healthScore,
    sub_scores: subScores,
    score_explanation: getHealthScoreExplanation(healthScore, subScores),
    metrics: healthMetrics,
    recommendations: includeRecommendations ? generateHealthRecommendations(healthMetrics, subScores) : [],
...
function calculateSpendingScore(metrics: FinancialMetrics): number {
  ...
  if (metrics.overspent_categories && metrics.overspent_categories > 0) {
    const penalty = Math.min(50, metrics.overspent_categories * 8 + Math.pow(metrics.overspent_categories, 1.5) * 2);
    score -= penalty;
  }
...
function calculateDebtScore(metrics: FinancialMetrics): number {
  ...
  if ((metrics.debt_to_asset_ratio ?? 0) > 0) {
    if ((metrics.debt_to_asset_ratio ?? 0) > 60) score -= 40;  // Critical
    else if ((metrics.debt_to_asset_ratio ?? 0) > 40) score -= 25;  // Poor
...
function calculateBudgetDisciplineScore(metrics: FinancialMetrics): number {
  ...
  if ((metrics.budget_utilization ?? 0) > 105) score -= 30;      // Significantly over budget
  else if ((metrics.budget_utilization ?? 0) > 95) score -= 10;  // Slightly over budget  
  else if ((metrics.budget_utilization ?? 0) < 60) score -= 15;  // Under-utilizing budget
  
  // Unallocated funds penalty
  if ((metrics.unallocated_funds ?? 0) < -200) score -= 15;      // Large negative balance
...
function getHealthScoreExplanation(score: number, subScores?: HealthSubScores): string {
  ...
  if (score >= 90) explanation = 'Excellent financial health with strong discipline across all areas';
  ...
  if (subScores) {
    const weakestArea = Object.entries(subScores)
...
function generateHealthRecommendations(metrics: FinancialMetrics, subScores: HealthSubScores): string[] {
    ...
    if ((score as number) < 70) {
      switch (area) {
        case 'spending_health':
          if (metrics.overspent_categories && metrics.overspent_categories > 0) {
            recommendations.push(`🚨 Address ${metrics.overspent_categories} overspent categories immediately`);
          }
          if (metrics.underfunded_categories && metrics.underfunded_categories > 0) {
            recommendations.push(`📊 Budget for ${metrics.underfunded_categories} unfunded categories`);
          }
          break;
          
        case 'debt_health':
          if (metrics.debt_to_asset_ratio && metrics.debt_to_asset_ratio > 40) {
            recommendations.push(`💳 High debt ratio (${metrics.debt_to_asset_ratio.toFixed(1)}%) - prioritize debt reduction`);
        ...
        case 'emergency_fund_health': {
          ...
          if (currentAmount < 1000) {
            recommendations.push(`🛡️ Build emergency fund - currently $${currentAmount.toFixed(0)}, target $2500+`);
          } else if (currentAmount < 7500) {
            recommendations.push(`📈 Grow emergency fund to 6 months of expenses (currently $${currentAmount.toFixed(0)})`);
          }
          break;
        }
        ...
        case 'budget_discipline': {
          if (metrics.budget_utilization && metrics.budget_utilization > 100) {
            recommendations.push(`⚠️ Over budget by ${(metrics.budget_utilization - 100).toFixed(1)}% - review spending`);
          }
          if (metrics.unallocated_funds && metrics.unallocated_funds < -100) {
            recommendations.push(`🔄 Negative balance of $${Math.abs(metrics.unallocated_funds).toFixed(0)} needs addressing`);
          }
          break;
        }
      }
