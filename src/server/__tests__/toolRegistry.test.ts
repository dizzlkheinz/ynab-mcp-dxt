...
describe('ToolRegistry', () => {
  ...
  it('prefers custom metadata JSON schema when provided', () => {
    ...
    expect(found?.inputSchema).toBe(customSchema);
  });
  ...
  it('merges default arguments before validation', async () => {
    registry.register({
      ...
      defaultArgumentResolver: vi.fn(() => ({ id: 'resolved-id' })),
  ...
  it('extracts minify override from arguments when not explicitly provided', async () => {
    ...
    expect(dependencies.responseFormatter.runWithMinifyOverride).toHaveBeenCalledWith(
      false,
      expect.any(Function),
    );
  });
  ...
  it('returns validation error result for unknown tools', async () => {
    ...
    expect(result).toEqual(createResult('validation:Unknown tool: missing_tool:The requested tool is not registered with the server'));
  ...
  it('surfaces validation failures from security wrapper', async () => {
    ...
    expect(result.content[0]?.text?.startsWith('validation:Invalid parameters for sample_tool:')).toBe(true);
  ...
  it('normalizes unexpected security errors', async () => {
    ...
    expect(customDeps.errorHandler.handleError).toHaveBeenCalledWith(error, 'executing security_tool');
  });
  ...
  it('returns permissive schema when conversion fails', () => {
    ...
    expect(tool?.inputSchema).toEqual({ type: 'object', additionalProperties: true });
  });
